...
* 오버로딩의 장점
  ...
  ```
   * 근본적으로 같은 기능을 하는 메서드이지만, 작성하는 쪽에서나 사용하는 쪽에서 구분해서 기억하기 쉽지않다
   * 오버로딩을 통해 여러 메서드들이 println이라는 하나의 이름으로 정리될수있다
   * 메서드의 이름만 보고도 쉽게 알아볼수있음
   
* 가변인자
   * 매개변수 개수를 동적으로 지정해 줄 수 있는 기능
   * '타입... 변수명' 형식으로 선언한다
...
=>인스턴스를 생성하기위해 사용해왔던 '클래스이름()'이 생성자이다
```
...
* 기본 생성자
   * 컴파일러가 제공하는 것
   ...
   Card() {}
   ```
   * 특별히 인스턴스 초기화 작업이 요구되어지지 않는다면 생성자를 정의하지 않고 컴파일러가 제공하는 기본 생성자를 사용해도 된다
   * 기본 생성자가 컴파일러에 의해서 추가되는 경우는 클래스에 정의된 생성자가 하나도 없을 때 뿐이다
 
* 매개변수가 있는 생성자
   * 인스턴스마다 각기 다른 값으로 초기화되어야하는 경우가 많기 때문에 매개변수를 이용한 초기화가 매우 유용하다
   ```
   Car c = new Car();
   c.color = "white";
...
* 생성자를 이용한 인스턴스의 복사
   * 현재 사용중인 인스턴스와 같은 상태를 갖는 인스턴스를 하나 더 만들고자 할 때 생성자를 이용 할 수 있다
   ```
...
* 명시적 초기화
   * 변수를 선언과 동시에 초기화 하는 것
   ...
   class Car {
        ...
        Enhine e = new Engine();      // 참조형 변수의 초기화
   }
 
* 초기화 블럭
   * 복잡한 초기화 작업이 필요할 때는 사용함
   ```
   클래스 초기화 블럭 => 클래스변수의 복잡한 초기화에 사용된다
   인스턴스 초기화 블럭 => 인스턴스변수의 복잡한 초기화에 사용된다
   ```
   * 인스턴스 초기화 블럭은 클래스 내에 블럭{} 만들고 그 안에 코드만 작성
   * 클래스 초기화 블럭은 인스턴스 초기화 블럭 앞에 static만 덧붙임
...
인스턴스변수의 초기화순서 =>  기본값 - 명시적초기화 - 인스턴스 초기화 불럭 - 생성자
```
